...
- Partial mirror **не затирает** уже зеркалированные разделы при смене ветки TOC в том же `--out`; внутренние ссылки переписываются по `_catalog/pages.yaml`, в том числе на страницы из других прогонов.
